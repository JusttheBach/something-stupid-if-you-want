...
struct file *pget_file(int fd)
{
    if (fd < 0)
        return NULL;

    struct thread *curr = thread_current();
    if (fd >= curr->next_fd || curr->fdt[fd] == NULL)
        return NULL;

    return curr->fdt[fd];
}

struct page *page_valid(void *addr)
{
...
int isValid(void *ptr){
	...
	return ((ptr!= NULL) && (page_valid(ptr) == NULL) && (pml4_get_page(thread_current()->pml4, ptr)==NULL));
}
...
void buf_val(void *buf, size_t size, bool write) {
	...
	for (void *addr = end; addr >= start; addr -= PGSIZE) {
		struct page *page = page_valid(addr);
		if (page == NULL) {
			exit(-1);
		}
...
int fork (const char *thread_name) {
	...
	int result = process_fork(thread_name, &thread_current()->copy_if);
...
static void *mmap(void *addr, size_t length, int writable, int fd, off_t offset)
{
    ...
    {
        struct page *pg = page_valid(addr);
        if (pg != NULL)
        {
            return NULL;
        }
